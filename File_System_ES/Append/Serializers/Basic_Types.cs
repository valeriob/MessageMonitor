...
namespace File_System_ES.Append.Serializers
{
    public class Int_Serializer : ISerializer<int>
    {
        ...
        public unsafe void To_Buffer(int[] values, int end_Index, byte[] buffer, int buffer_offset)
        {
            ...
            fixed (byte* p_buff = &buffer[0])
            {
                ...
                File_System_ES.Append.Unsafe_Utilities.Memcpy(shifted, (byte*)p_Pointers, 4 * (end_Index + 1));
            }
        }
    ...
    public class Guid_Serializer : ISerializer<Guid>
    {
        ...
        unsafe public void To_Buffer(Guid[] values, int end_Index, byte[] buffer, int buffer_offset)
        {
            ...
            {
                Append.Unsafe_Utilities.Memcpy(p_buffer + buffer_offset, (byte*)p_values, 16 * end_Index);
            }
        }
        ...
        unsafe public Guid[] Get_Instances(byte[] buffer, int startIndex, int length)
        {
            ...
            fixed (byte* p_buffer = &buffer[0])
            {
                Append.Unsafe_Utilities.Memcpy((byte*)p_result, p_buffer, 16 * length);
            }
